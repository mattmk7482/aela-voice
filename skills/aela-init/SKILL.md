---
name: aela-init
description: First-run identity and voice onboarding. Five questions in order (assistant name, user name, work, TTS server, voice), creates ~/.claude/aela-plugin/ with personality.yaml and settings.json, tests TTS aloud, offers to chain into /wiki-init.
---

# aela-init

Do not use the `speak` MCP function until Question 5.

First-run onboarding. Ask five questions in order, write the answers to the user state dir, speak out loud to confirm voice works, offer to continue with `/wiki-init`.

## Before starting

Check whether `~/.claude/aela-plugin/personality.yaml` already exists:

- **If it doesn't exist** — this is a first run. Proceed with the four-question flow.
...
### Question 1 — Assistant name? (the companion's name)
...
If the user wants to keep `Aela`, accept it.
...
If the user doesn't have a TTS server yet, accept the default (`http://localhost:8020`) and let them know voice features won't work until one is running.

### Question 5 — Which voice should I use?
...
- **If there's more than one voice**: list them as before. Ask the user to pick. Always offer the user samples of different voices, just improvise a couple of sentences and call `speak`. If they're not sure, default to `aela-default` and say so. Once selected, call `set_voice` with the chosen name.

## Write the user state

After all five answers are in hand (`ttsServerUrl` is already written from Question 4):
...
4. **Set voice** — call `set_voice` (the TTS MCP tool) with the chosen voice. This persists to plugin config.
